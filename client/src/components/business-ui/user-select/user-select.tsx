...
function createUsersFetcher(
  options: { accountType?: AccountType; pageSize?: number } = {},
) {
...
export const UserSelect: React.FC<UserSelectProps> = (props) => {
  ...
  const baseFetchFn = useMemo(
    () => createUsersFetcher({ accountType }),
    [accountType],
  );

  // 正在开户中的用户 ID 集合
  const [convertingSet, setConvertingSet] = useState<Set<string>>(
    () => new Set(),
  );

  // 已开户用户映射：旧 ID → 转换后的 UserSelectItemValue，用于回写到待选列表
  const [convertedMap, setConvertedMap] = useState<
    Map<string, UserSelectItemValue>
  >(() => new Map());
  ...
  const handleChange = useCallback(
    (newValue: UserSelectItemValue | UserSelectItemValue[] | null) => {
      ...
      if (multiple && Array.isArray(newValue)) {
        const hasUnregistered = newValue.some((u) =>
          isUnregisteredExternalContact(u),
        );
        if (hasUnregistered) {
          pendingConvertRef.current = true;
        }
        const filtered = newValue.filter(
          (u) => !isUnregisteredExternalContact(u),
        );
          ...
          if (onChange) {
            if (multiple) {
              const current = Array.isArray(internalValueRef.current)
                ...
                : [];
              const externalValue = toExternalValue(
                [...current, convertedUser],
                multiple,
              );
              (onChange as (value: unknown) => void)(externalValue);
            } else {
